...
END;
            ...
            foreach ($_ as $contract_id) {
                if ($contract_id === -1) {
                    ...
                    $html = format($html, array("title" => $row["title"]. ($row["hidden"] ? ' (hidden)' : ""), "values" => generateValuesHtml($row["cash"], $row["awareness"], $dynamic) . "<span style='float: right; font-size: 95%'><i class='fas fa-download' title='Export (JSON)' data-toggle='tooltip'></i></span>", "description" => $row["description"] . $note, "categories" => generateCategoriesHtml(explode(',', $row["categories"])), "contract_id" => $contract_id));
